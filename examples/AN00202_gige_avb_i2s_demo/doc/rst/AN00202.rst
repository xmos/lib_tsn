...
the project via the ``USED_MODULES`` variable in the ``Makefile``::
 ...
 USED_MODULES = lib_otpinfo(>=2.0.0) lib_tsn(>=7.0.0) lib_i2s(>=2.0.1)
...
is used to link the Quad SPI flash library, which is bundled with the tools::
  ...
  XCC_FLAGS = -Os -save-temps -g -report -Xmapper --map -Xmapper MAPFILE -fxscope -DRGMII=1 -lquadflash
...
The ``avb_conf.h`` file provides configuration #defines for the AVB endpoint and override defaults in the
...
The ``par`` functionality describes several tasks running in parallel across multiple logical cores and 2 tiles.
...
The latencies vary with the speed of the link and therefor a speed parameter is also required. These values are
...
.. literalinclude:: main.xc
   :start-on: buffer_manager_to_i2s
   :end-on: mclk_select)
...
Note that:
  ...
  - The task takes the ``server`` side of the ``i2s_callback_if``
    ...
    into this task.
  - It also tasks the ``client`` side of connections to the I2C bus
...
``i2s_master_callback_if`` in ``i2s.h``. There are four callbacks:
...
The I2S mode and clock ratio fields of the ``i2s_config`` structure are first set
...
A Talker (source) and Listener (sink) stream is setup to use 8 channel, 24-bit MBLA formats and a one-to-one channel
...
#. Open the **Edit** perspective, select the ``AN00202_gige_avb_i2s_demo`` project in the Project Explorer
...
#. On successful connection, the device will appear as an 8 in/ 8 out Audio Device in the *Audio MIDI Setup* window.
